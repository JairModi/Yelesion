...
function Reveal({ children, className = '', delay = 0, id }) {
  ...
  return <section ref={revealRef} className={`reveal ${visible ? 'is-visible' : ''} ${className}`} data-reveal-delay={delay} id={id}>{children}</section>
}

function SiteNav({ portfolio = false }) {
  return (
    <nav className="nav" aria-label="Main navigation">
      <a className="brand-logo" href={portfolio ? "/" : "#top"} aria-label="Yelesion home"><img src="/Yelesion%20Logo.png" alt="Yelesion Software Solutions" /></a>
      <div className="nav-links">{portfolio ? <a href="/">Back home</a> : <><a href="#meaning">Our meaning</a><a href="#contact">Contact</a><a href="/portfolio">Portfolio</a></>}</div>
    </nav>
  )
}
  ...
  return (
    <main className="portfolio-page">
      ...
      <section className="portfolio-hero">
        <div><p className="eyebrow">The Yelesion collective <span>•</span> 04 / Our work</p><h1>A team of<br /><em>makers.</em></h1></div>
        <p className="portfolio-intro">A living shelf for the people, projects and perspectives moving Yelesion forward.</p>
      </section>
  ...
  return (
    <main className="company-page">
      ...
      <section className="hero" id="top">
        <div className="hero-motion" aria-hidden="true"><span></span><span></span><span></span><span></span></div>
...
export default function Root() {
  return window.location.pathname === '/portfolio' ? <PortfolioPage /> : <App />
}
